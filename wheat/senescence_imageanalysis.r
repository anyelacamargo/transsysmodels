...
pfs2cdata <- function(pfs, maxIntensity)
{
  ...
  {
    d <- data.frame(idtag = rep("dummyId", length(intensityList)), intensity = intensityList, 
                    time = rep(dstep, length(intensityList)), value = as.numeric(pfs[dstep, ]));
    if (is.null(cdata))
    {
      cdata <- d;
    }
...
showDistanceTable <- function(allData)
{
  ...
  {
    message(sprintf("distance(empirical, %s) = %6.2f", 
                    wheatModelName, allProfileCorrelationDistance(allData$doodleProfiles, 
                                                                  allData$lsysProfilesList[[wheatModelName]])));
  }
}
